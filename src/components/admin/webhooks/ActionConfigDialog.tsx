...
import { SendTemplateActionConfig } from './SendTemplateActionConfig';
...
export function ActionConfigDialog({
  ...
  const createLeadTag = useCreateLeadTag();
  ...
  const renderApplyTagsConfig = () => {
    ...
    const filtered = (leadTags || []).filter((t) => !q || t.name.toLowerCase().includes(q));
    const exactMatch = (leadTags || []).some((t) => t.name.trim().toLowerCase() === q);
    const canCreate = q.length > 0 && !exactMatch && !createLeadTag.isPending;

    const handleCreate = async () => {
      if (!canCreate) return;
      const created = await createLeadTag.mutateAsync({
        name: tagSearch.trim(),
        color: '#3B82F6',
      });
      if (created?.id) {
        setConfig((prev) => ({
          ...prev,
          tag_ids: Array.from(new Set([...(prev.tag_ids || []), created.id])),
        }));
        setTagSearch('');
      }
    };

    const selectedTags = (leadTags || []).filter((t) => selected.includes(t.id));
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Pesquise uma etiqueta existente ou digite um novo nome e crie na hora.
          </p>
          ...
          <div className="max-h-56 overflow-y-auto rounded-md border">
            ...
            {filtered.map((tag) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => toggleTagId(tag.id)}
                  className="flex items-center gap-2 w-full px-3 py-2 text-left text-sm hover:bg-muted transition-colors border-b last:border-b-0"
                >
            ...
            {canCreate && (
              <button
                ...
                className="flex items-center gap-2 w-full px-3 py-2 text-left text-sm hover:bg-muted transition-colors border-t bg-muted/30"
              >
                <Plus className="h-4 w-4 text-primary flex-shrink-0" />
                <span className="flex-1">
                  Criar etiqueta <strong>"{tagSearch.trim()}"</strong>
                </span>
              </button>
            )}
          </div>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            placeholder={"Ex:\n✅ Horário: Hoje às {horario_call}\n✅ Link: https://meet.google.com/xxx-xxxx-xxx\n\nPosso confirmar sua inscrição?"}
            value={config.ai_extra_context || ''}
            onChange={(e) => setConfig(prev => ({ ...prev, ai_extra_context: e.target.value }))}
            rows={5}
          />
          <p className="text-xs text-muted-foreground">
            O texto escrito aqui é seguido <strong>literalmente</strong> pelo agente: o formato e as quebras de linha
            são mantidos exatamente como você digitar. Escreva o exemplo da mensagem final.
          </p>
          <div className="rounded-md border bg-muted/40 p-2 space-y-1.5">
            <p className="text-xs text-muted-foreground">
              Clique para inserir uma variável. Ela é substituída pelo valor real salvo no lead antes do envio.
            </p>

            <div className="flex flex-wrap gap-1">
              {['nome', 'email', 'telefone', ...customFields.map((f: any) => f.field_key)].map((key) => (
                <Badge
                  key={key}
                  variant="secondary"
                  ...
                  onClick={() => setConfig(prev => ({
                    ...prev,
                    ai_extra_context: `${prev.ai_extra_context || ''}{${key}}`,
                  }))}
                >
                  {`{${key}}`}
                </Badge>
  ...
  const renderConfigFields = () => {
    switch (action.type) {
      ...
      case 'send_whatsapp_template':
        return <SendTemplateActionConfig config={config} setConfig={setConfig} />;
      case 'trigger_flow':
