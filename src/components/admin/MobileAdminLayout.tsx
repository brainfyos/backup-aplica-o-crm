...
import { ArrowLeft, Settings, Menu, LogOut } from 'lucide-react';
...
export function MobileAdminLayout({ 
  ...
  const canSeeItem = (item: AdminMenuItem) => !item.featureKey || effectivePlan?.features?.[item.featureKey] === true;
  const currentSection = allMenuItems.filter(canSeeItem).find(item => item.id === activeSection);
