...
function QuizOptionsFields({ opts, setOpt }: { opts: QuizChannelOptions; setOpt: (p: any) => void }) {
  return (
    <>
      <div className="space-y-1.5">
        <Label>Largura do conteúdo ({opts.max_width ?? 560}px)</Label>
        <Slider min={360} max={840} step={20} value={[opts.max_width ?? 560]} onValueChange={([v]) => setOpt({ max_width: v })} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label>Posição vertical</Label>
          <Select value={opts.vertical_alignment ?? 'center'} onValueChange={(v) => setOpt({ vertical_alignment: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-1.5">
          ...
          <Select value={opts.content_alignment ?? 'left'} onValueChange={(v) => setOpt({ content_alignment: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="left">Esquerda</SelectItem>
              <SelectItem value="center">Centro</SelectItem>
      ...
      <div className="space-y-1.5">
        ...
        <Select value={opts.surface ?? 'plain'} onValueChange={(v) => setOpt({ surface: v })}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="plain">Limpo, sobre o fundo</SelectItem>
            <SelectItem value="card">Cartão central</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div className="space-y-1.5">
        <Label>Largura do logo ({opts.logo_width ?? 128}px)</Label>
        <Slider min={64} max={240} step={8} value={[opts.logo_width ?? 128]} onValueChange={([v]) => setOpt({ logo_width: v })} />
      </div>
      <div className="flex items-center justify-between"><Label>Mostrar logo no cabeçalho</Label><Switch checked={opts.show_logo !== false} onCheckedChange={(v) => setOpt({ show_logo: v })} /></div>
      <div className="flex items-center justify-between"><Label>Mostrar progresso</Label><Switch checked={opts.show_progress !== false} onCheckedChange={(v) => setOpt({ show_progress: v })} /></div>
      <div className="flex items-center justify-between"><Label>Mostrar botão voltar</Label><Switch checked={opts.show_back_button !== false} onCheckedChange={(v) => setOpt({ show_back_button: v })} /></div>
      <div className="space-y-1.5">
