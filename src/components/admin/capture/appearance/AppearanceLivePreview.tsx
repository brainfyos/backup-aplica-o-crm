...
import { Bot, Send, MessageCircle, ChevronRight, ChevronLeft, CheckCircle2, Smile, Paperclip } from 'lucide-react';
...
function QuizPreview({ a }: { a: ChannelAppearance }) {
  ...
  return (
    <div className="h-full overflow-auto" style={applyAppearance(a)}>
      <div className={cn(
        'mx-auto min-h-full flex flex-col px-5 py-6 sm:px-8 sm:py-10',
        (opts.vertical_alignment ?? 'center') === 'center' ? 'justify-center' : 'justify-start',
      )}>
        <div
          className={cn(
            'w-full mx-auto flex flex-col',
            opts.surface === 'card' && 'p-5 sm:p-7 border',
        ...
        >
        {((opts.show_logo !== false && a.logo_url) || opts.show_back_button !== false) && (
          <div className="grid grid-cols-[36px_1fr_36px] items-center mb-5">
            {opts.show_back_button !== false ? <ChevronLeft className="h-4 w-4" /> : <span />}
            {opts.show_logo !== false && a.logo_url ? (
              <img
                src={a.logo_url}
                alt=""
        ...
        {opts.show_progress !== false && (
          <div className="h-1.5 rounded-full mb-6 overflow-hidden" style={{ background: trackBg }}>
