...
import { AgentTransferRules } from './AgentTransferRules';
...
import { useAgentConnections } from '@/hooks/useProductAgents';
...
import { AgentVoiceChannelTab } from './AgentVoiceChannelTab';
import { DEFAULT_VOICE_BEHAVIOR_PROMPT } from './AgentPromptTemplates';
import { AgentIntelligenceTab } from './AgentIntelligenceTab';
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const slug = String(agentRow.name || 'agente')
                        .toLowerCase()
                        .normalize('NFD')
                        .replace(/[\u0300-\u036f]/g, '')
                        .replace(/[^a-z0-9]+/g, '-')
                      ...
                      const filename = `agente-${slug}-${date}.json`;
                      ...
                      console.error('[export-agent]', err);
                      toast.error(err?.message ?? 'Erro ao exportar agente');
                    }
                  }}
                  className="gap-2"
                >
          ...
          <div className="mt-4 shrink-0 overflow-x-auto px-6">
            {formData.agent_type === 'admin' ? (
              ...
              <TabsList className="inline-flex h-auto w-max gap-1">
                ...
                <TabsTrigger value="intelligence" className="text-xs whitespace-nowrap">
                ...
                <TabsTrigger value="voice" className="text-xs whitespace-nowrap">
            ...
            <div className="px-6 py-4">
              ...
              <TabsContent value="behavior" className="mt-0 space-y-6">
                ...
                <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-4">
                  ...
                  {/* Aviso quando não há apresentação global nem nenhuma regra com despedida */}
                  {!formData.handoff_incoming_message?.trim() &&
                    !(Array.isArray((formData.tool_configs as any)?.transfer_rules)
                      ? (formData.tool_configs as any).transfer_rules.some((r: any) => r?.farewell?.trim())
                      : false) && (
                      <div className="rounded-md border border-destructive/50 bg-destructive/10 p-3 text-xs text-destructive flex items-start gap-2">
                        ...
                        <div>
                          ...
                          Sem apresentação e sem despedida nas regras, a troca de agente acontece em
                          silêncio e o lead pode ficar perdido.
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground italic">
                      Apresentação padrão deste agente ao assumir qualquer conversa. As demais
                      configurações (despedida, atrasos e resumo) ficam dentro de cada regra abaixo.
                    </p>
                  </div>
              ...
              {/* Tab: modelo, custo, contexto e CTAs permitidos */}
              <TabsContent value="intelligence" className="mt-0">
                <AgentIntelligenceTab
                  formData={formData}
                  onChange={(updates) => setFormData(prev => ({ ...prev, ...updates }))}
                />
              </TabsContent>
              ...
              {/* Tab: Canal Voz — regras aplicadas apenas em ligações */}
              <TabsContent value="voice" className="mt-0">
                <AgentVoiceChannelTab
                  formData={formData}
                  onChange={(patch) => setFormData(prev => ({ ...prev, ...patch }))}
                />
              </TabsContent>
