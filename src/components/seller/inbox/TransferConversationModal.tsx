...
export function TransferConversationModal({
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      // A conversa, a carteira do lead e os históricos precisam mudar juntos.
      // A RPC mantém a RLS restritiva e valida origem e destino na mesma empresa.
      if (transferType === 'user') {
        const composedUserNote = [internalNote || null, connectionNote].filter(Boolean).join(' • ') || null;
        const { error: transferUserError } = await supabase.rpc('transfer_conversation_to_user', {
          p_conversation_id: conversationId,
          p_to_user_id: selectedUserId!,
          p_internal_note: composedUserNote ?? undefined,
          p_evolution_instance_id: newInstance?.id,
        });

        if (transferUserError) throw transferUserError;
        ...
        await Promise.all([
          ...
          queryClient.invalidateQueries({ queryKey: ['lead-transfer-history'] }),
        ]);

        toast({
          title: 'Conversa transferida',
          description: newInstance
            ? `A conversa e o lead foram transferidos. Conexão alterada para ${newInstance.name}.`
            : 'A conversa e o lead foram transferidos para a nova carteira.',
        });
      ...
      const { error: transferError } = await supabase
        ...
        .insert({
          ...
          // A transferência para usuário já retornou acima via RPC.
          to_user_id: null,
