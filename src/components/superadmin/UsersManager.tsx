...
export function UsersManager() {
  ...
  const handleSetPassword = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.functions.invoke('super-admin-manage-user', {
        body: { action: 'set_password', user_id: pwUser.id, password },
      });
      ...
      toast.error(e?.message || 'Falha ao redefinir a senha');
    } finally {
      setSaving(false);
    }
  };
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case 'manager':
        return <Badge variant="secondary">Gestor (legado)</Badge>;
      case 'seller':
        return <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20">Usuário</Badge>;
      default:
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="seller">Usuário</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardContent>
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => { setPwUser(user); setPassword(''); setConfirmPassword(''); }}
                      >
      ...
      <Dialog open={!!pwUser} onOpenChange={(o) => !o && setPwUser(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Defina uma nova senha para {pwUser?.full_name || pwUser?.email}. O acesso é imediato.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPwUser(null)} disabled={saving}>
              Cancelar
            </Button>
