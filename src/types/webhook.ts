...
export interface WebhookActionConfig {
  ...
  template_connection_id?: string;
  template_id?: string;
  template_variable_mapping?: Record<string, { source: string; hint?: string; static_value?: string }>;
  /** Conversa sem agente de IA após o envio (só mensagens programadas) */
  template_no_agent?: boolean;
  /** Ações por botão do template: chave = texto do botão */
  template_button_actions?: Record<string, {
    tag_id?: string | null;
    cadence_id?: string | null;
    opt_out?: boolean;
    stop_cadence?: boolean;
    reply_mode?: 'none' | 'text' | 'ai';
...
export const ACTION_TYPES: Record<WebhookActionType, { label: string; description: string; icon: string }> = {
  ...
  send_whatsapp_template: {
    label: 'Enviar Template (WhatsApp Oficial)',
    description: 'Dispara um template HSM aprovado pela Meta e programa ações por botão',
    icon: 'MessageCircle'
  }
};
