 ...
 */
/**
 * Opções canônicas da query de branding.
 * Compartilhadas por `usePlatformBranding`, `usePlatformName` e `Logo` para que
 * todos os consumidores usem exatamente o mesmo cache — evitando dezenas de
 * refetches por navegação/foco (causa relevante de carga no backend).
 * As alterações no Super Admin continuam instantâneas porque o save invalida
 * explicitamente `PLATFORM_BRANDING_QUERY_KEY`.
 */
export const PLATFORM_BRANDING_QUERY_OPTIONS = {
...
export function usePlatformBranding() {
  const { data: settings } = useQuery(PLATFORM_BRANDING_QUERY_OPTIONS);
