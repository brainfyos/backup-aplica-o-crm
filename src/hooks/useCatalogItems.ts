  ...
  return useMutation({
    ...
    onSuccess: (data: any) => {
      toast({
        ...
        description: `${data.items_created || 0} novos · ${data.items_updated || 0} atualizados · ${data.items_skipped || 0} páginas ignoradas · ${data.items_failed || 0} falhas`,
      });
    },
...
export function useCatalogRevalidateMedia() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: { organization_id: string; product_id?: string | null }) => {
      const { data, error } = await supabase.functions.invoke('catalog-revalidate-media', { body: input });
      if (error) throw error;
      return data;
    },
    onSuccess: (data: any) => {
      ...
      toast({
        title: 'Imagens revalidadas',
        description: `${data.checked || 0} itens verificados · ${data.cleaned || 0} corrigidos · ${data.without_image || 0} sem foto válida`,
      });
    },
    onError: (e: any) => toast({ title: 'Falha ao revalidar', description: e.message, variant: 'destructive' }),
  });
}
