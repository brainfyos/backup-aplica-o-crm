...
export interface TeamInvitation {
  ...
  token: string;
  status: 'pending' | 'processing' | 'accepted' | 'expired' | 'cancelled';
  state?: 'pending' | 'processing' | 'accepted' | 'expired' | 'invalid';
  expires_at: string;
  created_at: string;
  squad?: {
    id: string;
...
export function useInvitationByToken(token: string | null) {
  return useQuery({
    ...
    refetchInterval: (query) => {
      const data = query.state.data as TeamInvitation | null | undefined;
      return data?.state === 'processing' ? 1500 : false;
    },
  });
}
