...
export function useTeamMembers(organizationId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Coleta IDs de usuários vinculados à empresa via:
      // 1) profiles.organization_id (empresa "ativa" do usuário)
      // 2) user_organizations (vínculos multi-empresa — usuários reusados)
      const [primaryRes, linkedRes] = await Promise.all([
        supabase.from('profiles').select('id').eq('organization_id', orgId),
        supabase.from('user_organizations').select('user_id').eq('organization_id', orgId),
      ]);
      // `profiles` is the source of truth. `user_organizations` is a secondary
      // link table: in a freshly remixed project it can be empty or blocked by
      // RLS, and a failure there must never blank out the whole team list.
      if (primaryRes.error) throw primaryRes.error;
      if (linkedRes.error) {
        console.warn('[useTeamMembers] user_organizations indisponível:', linkedRes.error.message);
      }


      const idSet = new Set<string>([
        ...(primaryRes.data || []).map((p: any) => p.id),
        ...(linkedRes.data || []).map((l: any) => l.user_id),
      ]);
      const allIds = Array.from(idSet);
      if (allIds.length === 0) return [] as TeamMember[];
      ...
      // Only roles are essential for rendering a member. Squads and product
      // assignments are enrichment — degrade gracefully instead of failing.
      if (rolesResult.error) throw rolesResult.error;
      if (squadMembershipsResult.error) {
        console.warn('[useTeamMembers] squad_members indisponível:', squadMembershipsResult.error.message);
      }
      if (assignmentsResult.error) {
        console.warn('[useTeamMembers] user_product_assignments indisponível:', assignmentsResult.error.message);
      }
...
export function useRemoveTeamMember() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      ...
      if (error) throw error;
      if (data && (data as any).ok === false) throw new Error((data as any).error || 'Erro ao excluir usuário');
      return data as { mode?: string; remaining_organizations?: number };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['team-members'] });
      queryClient.invalidateQueries({ queryKey: ['product-assignments'] });
