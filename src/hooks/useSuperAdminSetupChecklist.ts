    ...
    queryFn: async () => {
      const [settingsRes, plansRes, orgsRes] = await Promise.all([
        supabase
          ...
          .select(
            'platform_name, logo_url, support_email, evolution_go_url, default_password_changed, remix_setup_completed, ai_validated_at, ai_mode'
          )
          .maybeSingle(),
      ...
      const aiMode = (s?.ai_mode ?? 'lovable') as 'lovable' | 'own_key' | 'per_org';
      const items: ChecklistItem[] = [
        ...
        {
          id: 'ai',
          label: aiMode === 'per_org' ? 'Modo de IA definido' : 'Validar IA da plataforma',
          description:
            aiMode === 'per_org'
              ? 'Cada empresa cadastra a própria chave em Integrações → Roteamento de IA.'
              : 'Faça um teste real de resposta da IA antes de cadastrar empresas.',
          done: aiMode === 'per_org' || !!s?.ai_validated_at,
          required: true,
          navigateTo: 'ai-keys',
        },
        {
