...
} from "./platform-email-templates.ts";
...
export async function sendPlatformEmail(args: SendPlatformEmailArgs): Promise<{ ok: boolean; error?: string }> {
  ...
  let tpl;
  try {
    tpl = await loadActivePlatformEmailTemplate(supabase, args.slug);
  } catch (error) {
    console.error("Failed to load platform email template", { slug: args.slug, error });
    return { ok: false, error: `Falha ao carregar template '${args.slug}'` };
  }
  ...
  const subject = renderPlatformEmailSubject(tpl.subject, vars);
  const html = renderPlatformEmailHtml(tpl.html_content, vars);
