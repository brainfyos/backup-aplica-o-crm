...
export function splitIntoBubbles(text: string, cfg: SplittingConfig = {}): string[] {
  ...
  // Unidades: cada linha do texto é uma unidade base; linhas longas são
  // quebradas por sentença. Unidades de linhas diferentes são unidas com "\n"
  // (preserva o layout), unidades da mesma linha com " ".
  ...
  lines.forEach((line, idx) => {
    for (const part of splitSentences(line)) units.push({ text: part, line: idx });
  });
  ...
  for (const u of units) {
    ...
    const joiner = u.line === currentLine ? ' ' : '\n';
    if ((current.length + joiner.length + u.text.length) <= targetLen) {
      current += joiner + u.text;
    } else {
...
export function buildHumanizationPromptBlock(cfg?: HumanizationConfig | null): string {
  ...
  if (t.slang && t.slang.length > 0) {
    out.push(
      `Gírias do seu repertório (use no MÁXIMO 1 a cada 3 mensagens, e só quando o tom for realmente descontraído): ${t.slang.slice(0, 20).join(', ')}.` +
      `\n🚫 NÃO use gíria quando estiver respondendo diretamente a uma resposta objetiva do lead (ex.: "sim", "não", "recorrência"), em objeção, preço, prazo ou assunto sério.`
    );
  ...
  out.push(
    '\n🧠 MEMÓRIA DA CONVERSA (regra inegociável):' +
    '\n- Releia o histórico antes de responder. NUNCA repita uma pergunta que o lead já respondeu, nem reformulada.' +
    '\n- Se o lead já respondeu (ex.: "ambos", "recorrência", "não assisti"), USE essa resposta e avance para a próxima etapa do SPIN.' +
    '\n- Se você já se apresentou, não se apresente de novo. Se já explicou algo, não reexplique — aprofunde ou avance.' +
    '\n- Se aparecerem mensagens suas repetidas no histórico, trate como uma só e siga em frente.'
  );
