...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (scopeParam === 'organization' && row.organization_id) {
      try {
        if (tagEventType) {
          ...
          leadId = await resolveOrCreateLead(admin, row);
          if (leadId) {
    ...
    const commerceStatus = commerceStatusFromLifecycle(tagEventType);
    if (scopeParam === 'organization' && row.organization_id && commerceStatus) {
      try {
        await reconcileCommerceSale(admin, {
          organizationId: row.organization_id,
          provider: 'cakto',
          externalOrderId: String(row.cakto_id),
          status: commerceStatus,
          leadId,
          productId: row.product_id ?? null,
          value: row.amount ?? 0,
          currency: 'BRL',
          paidAt: row.paid_at ?? (commerceStatus === 'confirmed' ? new Date().toISOString() : null),
          rawData: row.raw_payload ?? order,
        });
      } catch (saleErr) {
        console.error('[cakto-webhook] commerce reconciliation error', saleErr);
      }
    }
