  ...
  try {
    switch (name) {
      ...
      case "get_pipeline_summary": {
        ...
        const { data: pipelineProducts } = await supabase
          .from("products")
          .select("id")
          .eq("organization_id", orgId);
        const pipelineProductIds = (pipelineProducts ?? []).map((product: any) => product.id);
        const { data: stages } = pipelineProductIds.length
          ? await supabase
              .from("pipeline_stages")
              .select("id, name, order_index")
              .in("product_id", pipelineProductIds)
              .order("order_index")
          : { data: [] };
