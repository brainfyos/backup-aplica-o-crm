...
DECLARE
  ...
  v_current_org uuid;
  v_user_id uuid := accept_invitation.user_id;
BEGIN
  SELECT ti.* INTO inv FROM public.team_invitations AS ti
  WHERE ti.token = accept_invitation.invitation_token
  ...
  SELECT p.organization_id INTO v_current_org
  ...
  IF NOT EXISTS (
    ...
    WHERE p.id = v_user_id AND lower(p.email) = lower(inv.email)
  ) THEN
    RETURN FALSE;
  END IF;

  INSERT INTO public.user_organizations (user_id, organization_id, role, joined_via, is_default)
  VALUES (v_user_id, inv.organization_id, inv.role, 'invite', v_current_org IS NULL)
  ON CONFLICT (user_id, organization_id) DO UPDATE SET role = EXCLUDED.role;

  IF v_current_org IS NULL OR v_current_org = inv.organization_id THEN
    UPDATE public.profiles AS p SET organization_id = inv.organization_id WHERE p.id = v_user_id;
    INSERT INTO public.user_roles (user_id, role) VALUES (v_user_id, inv.role) ON CONFLICT DO NOTHING;
    PERFORM public.initialize_user_permissions(v_user_id, inv.organization_id, inv.role::text);
  END IF;
  
  -- Add to squad if specified
  IF inv.squad_id IS NOT NULL THEN
    INSERT INTO public.squad_members (squad_id, user_id, role)
    VALUES (inv.squad_id, v_user_id, 'member')
    ON CONFLICT DO NOTHING;
  END IF;

  UPDATE public.team_invitations AS ti SET status = 'accepted' WHERE ti.id = inv.id;
